...
function sortTable(n) {
  ...
  table = document.getElementById("gpx_table");
  switching = true;
    ...
    for (i = 1; i < (rows.length - 1); i++) {
      ...
      if (dir == "asc") {
		if (n != 4) {
			...
			// Enlever le km de la chaine de charactères pour comparer les nombres
			x_num = parseFloat(x.innerHTML.replace(/[^0-9\.]/g, ''));
			y_num = parseFloat(y.innerHTML.replace(/[^0-9\.]/g, ''));
			if (x_num > y_num) {
				shouldSwitch = true;
				break;
			}
